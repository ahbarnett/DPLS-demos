function [u ux uy do imv] = lapDevalclose(x, s, tau, side,CDLP) 
...
N = numel(s.x);
%for i=1:N, j = [1:i-1, i+1:N]; vb(i) = sum((tau(j)-tau(i))./(s.x(j)-s.x(i)).*s.cw(j)) + taup(i)*s.w(i)/s.sp(i);end

%vb2=0*s.x;
%for i=1:N, j = [1:i-1, i+1:N]; vb2(i) = sum((tau(j)*0-0*tau(i))./(s.x(j)-s.x(i)).*s.cw(j));end

if nargin==4
Sj=ones(N,1)*s.x.';
Wj=ones(N,1)*s.cw.';
CDLP=Wj./(Sj-Sj.');
CDLP(diag(true(N,1)))=0;
end
vb2=([ones(N,1),tau].*(CDLP*[tau,ones(N,1)]))*[1;-1];
vb=vb2+taup.*s.w./s.sp;
%vb2=Sij*tau-tau.*(Sij*ones(N,1))+taup.*s.w./s.sp;
