function z=SLP_Close_Apply(NPt,s,t,f,CSLP)
...
if isfield(NPt,'s')
    ...
    for k=1:M
        if ~isempty(NPt.s{k})
            ...
            sin=sum(in);
            if sin>0
                zz(in)=StokesScloseeval(xx.x(in),ss,ff,'i');
            end
            if sin<length(in)
                if nargin==5
                    zz(~in)=StokesScloseeval(xx.x(~in),ss,ff,'e',CSLP{k});
                else
                    zz(~in)=StokesScloseeval(xx.x(~in),ss,ff,'e');
                end
            end
            SLP=SLPmatrix(xx,ss,1)*[real(ff);imag(ff)];
            zz=zz-SLP(1:end/2)-1i*SLP(end/2+1:end);
